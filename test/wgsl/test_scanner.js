...
import { Scanner, Token, Keyword } from "../../wgsl/wgsl_scanner.js";
...
group("Scanner", function() {
    ...
    test("123u", function(test) {
        const scanner = new Scanner("123u");
        const tokens = scanner.scanTokens();
        test.equals(tokens.length, 2);
    ...
    test("0x123u", function(test) {
        const scanner = new Scanner("0x123u");
        const tokens = scanner.scanTokens();
        test.equals(tokens.length, 2);
        test.equals(tokens[0]._type, Token.uint_literal);
    ...
    test("0x.5", function(test) {
        const scanner = new Scanner("0x.5");
        const tokens = scanner.scanTokens();
        test.equals(tokens.length, 2);
    ...
    test("1<2||a>>2==0", function(test) {
        ...
        const scanner = new Scanner("a<2||a>>2==0");
        const tokens = scanner.scanTokens();
    ...
    test("array<vec4<f32>>", function(test) {
        ...
        const scanner = new Scanner("array<vec4<f32>>");
        const tokens = scanner.scanTokens();
    ...
    test("fn foo(a, b) -> d { return; }", function(test) {
        const scanner = new Scanner(`fn foo(a, b) -> d {
            // skip this comment
            return;
        }`);
        const tokens = scanner.scanTokens();
