...
import { Token, TokenType, TokenTypes, WgslScanner } from './wgsl_scanner';
...
export class WgslParser
{
  ...
  _assignment_statement(): AST.Assign | null
  {
    ...
    const isUnderscore = this._match(TokenTypes.tokens.underscore);
    ...
    return new AST.Assign(
      AST.assignOperatorParse(type.lexeme),
  ...
  _global_override_decl(): AST.Override | null
  {
  ...
  _type_decl(): AST.Type | null
  {
    ...
    {
      const type = this._advance().toString();
    ...
    {
      const pointer = this._previous().toString();
    ...
    // texture_sampler_types
    const type = this._texture_sampler_types();
    if (type) return type;
  ...
  _attribute(): Array<AST.Attribute> | null
  {
    ...
    const attributes: Array<AST.Attribute> = [];
