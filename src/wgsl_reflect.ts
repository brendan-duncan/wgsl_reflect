...
export class VariableInfo {
    ...
    get type(): AST.Type | null { return this.node.type; }

    get attributes(): Array<AST.Attribute> | null { return this.node.attributes; }
}
...
export class FunctionInfo {
    ...
    get returnType(): AST.Type | null { return this.node.returnType; }

    get args(): Array<AST.Argument> { return this.node.args; }
...
export class WgslReflect {
    ...
    initialize(code: string) {
        ...
        for (const node of this.ast) {
            if (node.astNodeType == "struct")
                this.structs.push(node as AST.Struct);

            if (node.astNodeType == "alias")
                this.aliases.push(node as AST.Alias);
    ...
    _getInputs(args: Array<AST.Argument>, inputs: Array<InputInfo> | undefined = undefined): Array<InputInfo> {
        if (inputs === undefined)
            inputs = [];
    ...
    _getInputInfo(node: AST.Member): InputInfo | null {
        ...
        if (location !== null) {
            ...
            const info = new InputInfo(node.name, node.type, node, location.name,
    ...
    getStructInfo(node: AST.Struct | AST.Var | null): BufferInfo | null {
        ...
        for (let mi = 0, ml = struct.members.length; mi < ml; ++mi) {
            ...
            const s = this.getStruct(type) || (isArray ? this.getStruct(type['format']?.name)
                : null)
            const isStruct = !!s;
            const si = isStruct ? this.getStructInfo(s) : undefined;
            const arrayStride = si?.size ?? isArray ? this.getTypeInfo(type['format'])?.size
    ...
    static readonly textureTypes = TokenTypes.any_texture_type.map((t) => { return t.name; });
    static readonly samplerTypes = TokenTypes.sampler_type.map((t) => { return t.name; });
}
